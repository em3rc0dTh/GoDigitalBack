// src/models/tenant/Transaction_Raw_C_Gmail.ts
import mongoose from "mongoose";
import { getOrCreateModel, getTenantDB } from "../../config/tenantDb";

export interface TransactionRawCGmailDocument extends mongoose.Document {
...
const TransactionRawCGmailSchema = new mongoose.Schema(
    {
    ...
    {
        timestamps: true,
        collection: "Transaction_Raw_C_Gmail", // Standardized Name
        strict: true,
    }
);
// TransactionRawCGmailSchema.index({ messageId: 1 }, { unique: true });
TransactionRawCGmailSchema.index({ matchStatus: 1, receivedAt: -1 });
TransactionRawCGmailSchema.index({ "routing.entityId": 1 });
TransactionRawCGmailSchema.index({ masterId: 1 });

export async function getTransactionRawCGmailModel(tenantId: string, detailId: string) {
    const systemDB = await getTenantDB(tenantId, detailId);
    return getOrCreateModel(
        systemDB,
        "Transaction_Raw_C_Gmail",
        TransactionRawCGmailSchema
    );
}
